...
class App:
	...
	###Frame 2 SWITCH / CONTROL PANEL SECTION###			
		...
		#Panel background image (needs to be defined before creating the Label
		self.switchpanel_background_img_raw = Image.open('{0}/metal_black.jpg'.format(config['content_folder']))
		self.switchpanel_background_img_raw = self.switchpanel_background_img_raw.resize((1,1),Image.ANTIALIAS)		
		self.switchpanel_background_img = ImageTk.PhotoImage(self.switchpanel_background_img_raw)
		
		#self.switch_panel = Label(self.frame2, image=self.switchpanel_background_img,bg='white')
		self.switch_panel = Label(self.frame2,bg='black')
		
		#self.switch_panel.place(x=0, y=0, relwidth=1, relheight=1)
		self.switch_panel.grid()
		...
		self.switch = tk.Button(self.switch_panel,width=1,height=1,command=self.switch_handler,relief=FLAT,activebackground='black')
	...
	def draw(self):	
		...
		#Drawing the button frame (requires different method to depthmap and video)
		if (self.switchpanel_background_img.width != self.switch_panel.winfo_width()) or (self.switchpanel_backgound_img.height != self.switch_panel.winfo_height()):
			self.switchpanel_background_img = Image.open('{0}/metal_black.jpg'.format(config['content_folder']))			
				
			self.temp_draw_image = self.resize(self.switchpanel_background_img,self.switch_panel,self.switchpanel_ratio)
			self.switchpanel_background_img = ImageTk.PhotoImage(self.temp_draw_image)
			self.switch_panel.configure(image=self.switchpanel_background_img)
			
			width = int(self.switch_panel.winfo_width()/5)
			height = int(width/3)
			self.switch.configure(width=width,height=height)		
		...
		# self.switch_panel = Label(self.frame2, image=self.switchpanel_background_img)
		# #self.switch_panel.place(x=0, y=0, relwidth=1, relheight=1)
		# # self.panel.grid()
	...
	def video_feed(self):           
		if self.connect.video_ready:
			...
			try:
				...
				##resize the image !!!!!!!!!!!!! Can this be achieved already with the draw function ????????
				...
				##process the raw frame #This needs testing with camera, could lines marked !!!! be removed ?
				cvimage = cv.cvtColor(frame, cv.COLOR_BGR2RGBA) #colours picture correctly              
				current_image = Image.fromarray(cvimage)#PIL, processes the matrix array                   
				imagetk = ImageTk.PhotoImage(image=current_image)  # convert image for tkinter#!!!!
				self.vid_show_image = imagetk  #assigns the processed image ready for drawing #!!!!          
