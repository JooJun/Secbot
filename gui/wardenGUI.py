...
class App:
	...
	###Frame 4 DEPTHMAP###
		...
		self.depmap_dis_img_path = '{0}/vid_disconnect.jpg'.format(config['content_folder'])
		self.depmap_dis_raw_img = Image.open(self.vid_dis_img_path)
	...
	def resize(self,image,frame):
		if frame.winfo_width()>1:    
			if frame.winfo_height() < int((frame.winfo_width()*0.5625)): 
				image = image.resize((
						int(frame.winfo_height()*1.77777) #image width
						,frame.winfo_height()#image height
						),
						Image.ANTIALIAS
						)                               
			else:
				image = image.resize((
						 frame.winfo_width()   #image width
						,int((frame.winfo_width()*0.5625))
						),#image height
						Image.ANTIALIAS
						)              
			return image
