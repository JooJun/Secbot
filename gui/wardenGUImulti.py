...
class App:
        ...
        ##create instance of other classes
                ...
                self.network, self.session, self.input_placeholder = self.warm_up_tf()
        ...
        def warm_up_tf(self):
            ...
            for x in range(1, 10):
                ...
                depthmap.depthmap_func(network, session, input_placeholder, self.depthmap_img_cam)
                end = timer()
                print('Time taken -cam : {}'.format(end-start))
            
            print('Warm up done')

            return network, session, input_placeholder
        ...
        def depmap_update(self):
                ...
                depthmap.depthmap_func(self.network, self.session, self.input_placeholder, self.depthmap_img_cam)
                print('depthmap done')
                ...
                if self.connect.video_ready:
                        ...
                        try:
                            ...
                            #(frame, cv.COLOR_BGR2RGBA) #colours picture correctly              
                                self.cvimage = cv.cvtColor(frame, cv.COLOR_BGRA2RGB) #colours picture correctly                      
